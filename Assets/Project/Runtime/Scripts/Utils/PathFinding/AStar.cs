...
{
	// public static List<Node> FindPath(Vector2Int startGlobal, Vector2Int targetGlobal, int searchRadius, List<TileTravelType> validTravelTypes, bool acceptNearest = false)
	// {
	// 	int searchGridSize = (searchRadius * 2) + 1;
	// 	Node[,] nodes = BuildNodeGrid(startGlobal, searchRadius, validTravelTypes, searchGridSize);
	...
	// 		{
	// 			if (!neighbour.CanTravel || closedSet.Contains(neighbour)) { continue; }
	...
	// 			if (nearTarget == null) 
	// 			{ 
	// 				if (neighbour.CanTravel) { nearTarget = neighbour; } 
	// 			}
	// 			else
	// 			{
	...
	// private static Node[,] BuildNodeGrid(Vector2Int startLoc, int searchRadius, List<TileTravelType> validTravelTypes, int searchGridSize)
	// {
	...
	// 			nodes[localX, localY] = new Node(globalLoc, new Vector2Int(localX, localY), MapBoard.Instance.IsPathable(globalLoc, validTravelTypes));
	// 		}
	// 	}
	// 	return nodes;
	// }
