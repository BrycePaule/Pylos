...
public class BuildingTableEntryDrawer: PropertyDrawer 
{
	...
	public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) 
	{
		...
		var travelTypesRect = new Rect(position.x, position.y + (lineHeight * 1) + (lineSpace * 1), position.width, lineHeight);

		var spriteRect = new Rect(position.x, position.y + (lineHeight * 2) + (lineSpace * 2), position.width, lineHeight);
		var iconRect = new Rect(position.x, position.y + (lineHeight * 3) + (lineSpace * 3), position.width, lineHeight);

		EditorGUI.PropertyField(idRect, property.FindPropertyRelative("ID"), GUIContent.none);
		EditorGUI.PropertyField(nameRect, property.FindPropertyRelative("Name"), GUIContent.none);
		EditorGUI.PropertyField(travelTypesRect, property.FindPropertyRelative("TravelTypes"), GUIContent.none);
